...
export const getMediaById = async (mediaId: number) : Promise<MediaSearchResult> => {
    console.log(mediaId);
    if (process.env.NODE_ENV === 'development') {
        return mockContent.find((media: MediaSearchResult) => media.mediaId === mediaId) as MediaSearchResult;
    } else {
